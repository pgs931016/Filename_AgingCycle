...
% load xlxs
Data_path = 'G:\공유 드라이브\BSL_Data2\HNE_AgingDOE';
...
% "Data"가 포함된 인덱스를 찾음
index1 = find(strcmp('HNE_AgingDOE', splitPath), 1);

% "Data"를 "Processed_Data"로 변경 (요구사항에 따라)
splitPath{index1} = 'HNE_AgingDOE_Raw';
...
for i = 1:length(subfolders)
    if strcmp(subfolders(i).name, desiredString)
        ...
        txtfiles = dir(fullfile(targetfolderPath, '*.txt'));
